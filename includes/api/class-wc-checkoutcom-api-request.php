...
require 'class-wc-checkoutcom-utility.php';
...
class WC_Checkoutcom_Api_request {
	...
	public static function create_payment( WC_Order $order, $arg, $subscription = null ) {
		...
		$request_param = self::get_request_param( $order, $arg, $subscription );
	...
	private static function get_request_param( WC_Order $order, $arg, $subscription = null ) {
		...
		$customerAddress = self::customer_address( $postData );
		...
		if ( $postData['payment_method'] == 'wc_checkout_com_cards' ) {
			...
			$method = self::get_apm_method( $postData, $order, $arg );
		...
		// Pay Order Page
		$isPayOrder = ! empty( $getData['pay_for_order'] ) ? (bool) $getData['pay_for_order'] : false;

		if ( $isPayOrder ) {
			if ( ! empty( $getData['order_id'] ) ) {
				$order_id = $getData['order_id'];
			} elseif ( ! empty( $getData['key'] ) ) {
	...
	private static function customer_address( $data ) {
		// Pay Order Page
		$isPayOrder = ! empty( $_GET['pay_for_order'] ) ? (bool) $_GET['pay_for_order'] : false;
	...
	public static function create_apm_payment( WC_Order $order, $payment_method ) {
		// Get payment request parameter
		$request_param = self::get_request_param( $order, $payment_method );
